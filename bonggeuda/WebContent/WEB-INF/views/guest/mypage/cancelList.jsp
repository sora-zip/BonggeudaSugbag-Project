...
<div class="container">
	...
	<div id="cont_area" style="padding-top: 30px;"><!-- 본문 콘텐츠-->
		<div class="cont_inner" id="policy_page" style="min-height: 435px; text-align: left; float: left; width: 100%;">
			...
			<div style="font-size: 15px; border: 1px solid; width: 100px; background: #d3d3d3; border-style: none; border-radius: 5px;">예약 취소</div>
			<div style="font-size: 30px; margin-bottom: 20px; text-align: left;"><c:out value="${ requestScope.userCancleContent.accomoName }"/> <c:out value="${ requestScope.userCancleContent.roomName }"/></div>
			<img src="${pageContext.servletContext.contextPath}/${ requestScope.userCancleContent.thumbnailPath }" style="width: 100%; margin-bottom: 50px;">

			<div class="reservation_information">
				<table style="margin-bottom: 30px; width: 90%;">
					<tr>
						<td>체크인</td>
						<td><c:out value="${ requestScope.userCancleContent.bookCheckDate }"/> &nbsp <c:out value="${ requestScope.userCancleContent.bookCheckIn }"/></td>
					</tr>
					<tr>
						<td>체크아웃</td>
						<td><c:out value="${ requestScope.userCancleContent.checkoutDate }"/> &nbsp <c:out value="${ requestScope.userCancleContent.checkOut }"/></td>
					</tr>
					<tr>
						<td>예약번호</td>
						<td><c:out value="${ requestScope.userCancleContent.bookNo }"/></td>
					</tr>
					<tr>
						<td>예약자 이름</td>
						<td><c:out value="${ requestScope.userCancleContent.bookUserName }"/></td>
					</tr>
					<tr>
						<td>전화번호</td>
						<td><c:out value="${ requestScope.userCancleContent.bookPhone }"/></td>
					</tr>
				</table>
			</div>
			...
			<div style="text-align: left; float: left; font-size: 20px; margin-bottom: 30px; width: 100%;">
				<table style="width: 90%;">
					...
					<tr>
						<td>예약 취소 금액</td>
						<td style="color: #6eceda;"><c:out value="${ requestScope.userCancleContent.cancleAmount }"/>원</td>
					</tr>
				</table>
			</div>
