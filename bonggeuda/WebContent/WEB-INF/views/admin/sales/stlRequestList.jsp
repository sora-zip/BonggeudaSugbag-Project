...
<title>Settlement RequestList</title>
<link
		...
		<div class="container">
			<div class="blog-list">
				...
				<table class="table" style="width: 800px;">
					<div class="tab">
						<span class="tab_btn active" data-toggle="tab">정산 요청 업체 리스트</span>

						<form id="sortForm"
							action="${ pageContext.servletContext.contextPath }/sales/stlrequest"
							method="get">

							<c:choose>
								<c:when test="${ !empty requestScope.stlStatus }">
									<select id="stlStatus" name="stlStatus">
										<option value="N"
											<c:if test="${requestScope.stlStatus eq 'N'}">selected</c:if>>정산대기</option>
										<option value="Y"
											<c:if test="${requestScope.stlStatus eq 'Y'}">selected</c:if>>정산완료</option>
									</select>
								</c:when>
								<c:otherwise>
									<select id="stlStatus" name="stlStatus">
			...
			<script>
				const link = "${ pageContext.servletContext.contextPath }/sales/stlrequest";
				const searchLink = "${ pageContext.servletContext.contextPath }/owner/search";

				if (document.getElementById("startPage")) {
					const $startPage = document.getElementById("startPage");
					$startPage.onclick = function() {
						location.href = link + "?currentPage=1&refundStatus=${requestScope.refundStatus}&stlStatus=${requestScope.stlStatus}";
					}
				}
				...
				if (document.getElementById("prevPage")) {
					...
					$prevPage.onclick = function() {
						location.href = link
								+ "?currentPage=${ requestScope.pageInfo.pageNo - 1 }&refundStatus=${requestScope.refundStatus}&stlStatus=${requestScope.stlStatus}";
					}
				}
				...
				if (document.getElementById("maxPage")) {
					...
					$maxPage.onclick = function() {
						location.href = link
								+ "?currentPage=${ requestScope.pageInfo.maxPage }&refundStatus=${requestScope.refundStatus}&stlStatus=${requestScope.stlStatus}";
					}
				}
				...
				function pageButtonAction(text) {
					location.href = link + "?currentPage=" + text + "&refundStatus=${requestScope.refundStatus}&stlStatus=${requestScope.stlStatus}";
				}
